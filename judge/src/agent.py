...
WEIGHTED_CATEGORIES: list[tuple[str, str, float]] = [
    ("cin_fusion", "zh", 100.0),
    ("cin_fusion", "en", 100.0),
    ("cin_fusion", "ja", 100.0),
    ("flip_overlay", "zh", 100.0),
    ("flip_overlay", "en", 100.0),
    ("flip_overlay", "ja", 100.0),
    ("gradient_overlay", "zh", 100.0),
    ("gradient_overlay", "en", 100.0),
    ("gradient_overlay", "ja", 100.0),
]
...
class Agent:
    ...
    async def run(self, message: Message, updater: TaskUpdater) -> None:
        ...
        weighted_summary = _weighted_score_summary(by_task_lang)
        result_data = {
            "total": totals["total"],
            ...
            "weighted_score": weighted_summary["weighted_score"],
            "weighted_score_max": weighted_summary["max_score"],
            "weighted_score_categories": weighted_summary["categories"],
...
def _weighted_score_summary(
    ...
    for task, lang, max_points in WEIGHTED_CATEGORIES:
        key = f"{task}:{lang}"
        bucket = buckets.get(key)
        accuracy = _accuracy(bucket) if bucket else 0.0
        score = accuracy * max_points
        total_score += score
        ...
        categories.append(
            {
                ...
                "correct": bucket["correct"] if bucket else 0,
                "accuracy": accuracy,
                "score": score,
            }
        )
...
def _build_summary_text(result: dict[str, Any]) -> str:
    ...
    weighted_score_max = result.get("weighted_score_max")
    weighted_line = ""
    if weighted_score is not None and weighted_score_max is not None:
        weighted_line = f"Weighted Score: {weighted_score:.2f}/{weighted_score_max:.0f}\n"
    return (
        "Benchmark Results\n"
        f"Samples: {total}\n"
